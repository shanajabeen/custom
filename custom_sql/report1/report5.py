...
class REPORT5Xlsx(models.AbstractModel):
    _name = 'report.custom_sql.report_id_xlx'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        domain =[]
        first=data['form']['date_from']
        last=data['form']['date_to']
        ...
        'am.amount_total_signed,am.amount_residual_signed,am.state,am.invoice_user_id,rp.name as name,co.name as company '\
        'from account_move am '\
        'join res_users ru on ru.id = am.invoice_user_id '\
        'join res_partner rp on ru.partner_id = rp.id '\
        'join res_partner pt on pt.id = am.partner_id '\
        'join res_company co on co.partner_id = am.company_id '\
        'where invoice_date '\
        'between %s '\
        'and %s ',                                   
        (str(first),str(last)))
        ...
        for each in values:
            ...
            worksheet.write('C%s' % new_row, each['customer_name'])
            worksheet.write('D%s' % new_row, each['invoice_date'].strftime("%d-%m-%Y"), center)
            worksheet.write('E%s' % new_row, each['company'])
